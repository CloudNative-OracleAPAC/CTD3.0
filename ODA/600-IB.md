...
> **Note:** If you are not using an Oracle Public Cloud instance for the Test Drive (such as the use of a hosted Docker Image), you will need to expose the Digital Assistant to the Internet. For this exercise we will be using the ngrok service to give us an externally accessible URL.  (The value for external URL will be given to you by your instructor).<br><br> Eg. ```https://db8e9999.ngrok.io/```<br><br>Replace the Base URL in the Webhook URL that you copied down with the **ngrok** entry point.<br>Hence,<br><br>```http://bots-connectors:8000/connectors/v1/tenants/botslc01ga/listeners/facebook/channels/...```<br><br>becomes<br><br>```https://db8e9999.ngrok.io/connectors/v1/tenants/botslc01ga/listeners/facebook/channels/...```
