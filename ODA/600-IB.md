...
**Note:**
If you are not using an Oracle Public Cloud instance for the Test Drive (such as the use of a hosted Docker Image), you will need to expose the Digital Assistant to the Internet. For this exercise we will be using the ngrok service to give us an externally accessible URL.  The value for external URL will be given to you by your instructor.
...
5 Specify for which Bot it will be associated, by selecting your *Skill* from the *Pop-List* on the **``Route To:``** field.
